...
class Feedforward(torch.nn.Module):
    def __init__(self, input_dim, hidden_sizes, output_size,learning_rate= 0.0002, activation_fun=torch.nn.Tanh(), output_activation=None,name="feedforward",folder="tmp",device="cpu"):
    ...
    def forward(self, x):
        if self.device.type == 'cuda' and not x.device.type == 'cuda':
            x = x.to(self.device)
