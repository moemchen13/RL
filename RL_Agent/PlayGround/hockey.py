...
def run_sac_agent_in_env_modes(agent,mode,log_interval,save_interval,max_episodes,
    ...
    for episode in range(1,max_episodes+1):
        ...
        if episode % save_interval == 0:
            ...
            save_statistics(rewards,lengths,q_losses,policy_losses,temperature_losses,mode,random_seed,episode)
...
def main():
    ...
    optParser.add_option('-n', '--name',action='store', type='string',
                         dest='name',default="SAC_run", help='Name for run')
    optParser.add_option('-m', '--mode',action='store', type='string',
                         dest='mode',default="Defense",
                         help='Possible modes to play (default %default) options Attack/Defense/self')
    optParser.add_option('-ti', '--train_iter',action='store',  type='int',
    ...
    optParser.add_option('-a', '--agent',action='store',  type='string',
                         ...
                         help='Choose an Agent you wanna actiavte (default %default) Options: SAC/DSAC/DR3')
    optParser.add_option('-o', '--opponent',action='store',  type='string',
                         dest='opponent',default='SAC',
                         help='Choose an opponent only in self (default %default) Options: SAC/DSAC/DR3')
    optParser.add_option('-i', '--info',action='show',  type='string',
                         help='This is the hockeytrain file from Team gradient descent also look at out github https://github.com/moemchen13/RL.git')
